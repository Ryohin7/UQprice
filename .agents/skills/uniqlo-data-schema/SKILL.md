...
* **範例輸出**:
  ...
  ```
### 系統內建次分類與動態篩選
除了自訂關聯的商品分類外，各主分類底下亦預設配置了以下幾種內建次分類，由前端基於商品欄位進行動態篩選：
* **全部商品 (`ALL`)**：顯示該主分類所屬性別的全部商品。
* **新品上市 (`new_arrival`)**：顯示 `isNewArrival === true` 的商品。
* **即將上市 (`coming_soon`)**：顯示 `firstListTime` 晚於當前時間 (未來上架) 的預訂販售商品。
* **限定價格 (`limited_price`)**：顯示行銷促銷標籤包含「限定價格」字樣的商品。
* **特價商品 (`sale_price`)**：顯示行銷促銷標籤包含「特價商品」字樣的商品。
